...
function candsTotal(s){ const out=[]; const m=s.match(/^(\d+)[,.](\d{1,2})$/); if(m) out.push([+(m[1]+'.'+m[2]),0]); const D=s.replace(/\D/g,''); if(!D) return out; out.push([+D,3]); if(D.length>=3) out.push([+(D.slice(0,-2)+'.'+D.slice(-2)),1]); if(D.length>=2) out.push([+(D.slice(0,-1)+'.'+D.slice(-1)),2]);
...
function candsQty(s){ const D=s.replace(/\D/g,''); const out=[]; if(D) out.push([+D,0]); const sw={'0':'8','8':'0','9':'0','6':'0','5':'6','1':'7','7':'1','2':'7','3':'8'}; for(let k=0;k<10;k++) out.push([+(D+k),3]); // потерянная последняя цифра
...
async function ocrInvoice(file, progress){
  ...
  for(let p=1;p<=pdf.numPages;p++){
    ...
    for(let i=0;i<G.rows.length-1;i++){
      ...
      const row={pg:p, i, code, desc:'', qty:res?res.qty:gq, price:res?res.price:gp, value:res?res.value:Math.round(gq*gp*100)/100, ocr:true, ok:!!res, raw};
